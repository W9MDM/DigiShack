...
// 200 with a status rather than 404 for "no match": an unknown entity is a normal
// answer about a callsign, not a failed request.
async function get(req: NextApiRequest, res: NextApiResponse) {
  const { callsign, at } = querySchema.parse(req.query);
  const result = await resolveDxcc(callsign, at ?? new Date());
  sendJson(res, 200, { callsign: callsign.toUpperCase(), ...result });
}
