 ...
 */
// ASYNC SINCE THE DECODER MOVED TO ITS OWN THREAD.
//
// `processWindow` used to decode inline and return with its events already emitted, so this
// helper could call it and read the results on the next line. It now hands the audio to a
// worker and resolves when that worker answers. Awaited rather than slept on: the promise
// IS the completion signal.
//
// This check is what caught the change - it was not in the set run while the thread was
// being built, and the public mirror's suite failed on it. Five round-trips reporting
// "0 decodes" was the decoder working perfectly and the test reading the answer too early.
async function runWindow(
  mode: TxMode,
  samples: Float32Array,
  inputRate = DAX_SAMPLE_RATE,
): Promise<{ decodes: Decode[]; rms: number; skipped: boolean }> {
  ...
  await pipeline.processWindow(Array.from(samples), new Date(0));
  pipeline.stop();
...
async function main(): Promise<void> {
  ...
  for (const mode of ["FT8", "FT4", "FT2"] as const) {
    ...
    const { decodes, skipped } = await runWindow(mode, liveWindow(mode, wave));
  ...
  console.log("\nthe same pipeline at the Icom's 48 kHz");
  {
    ...
    for (const mode of ["FT8", "FT2"] as const) {
      ...
      const { decodes } = await runWindow(mode, buf, rate);
      const hit = decodes.find((d) => d.message === MESSAGE);
  ...
  console.log("\nthe silence check");
  {
    ...
    const { skipped, decodes } = await runWindow("FT8", quiet);
  ...
  console.log("\nthe short-window guard");
  {
    ...
    const { decodes } = await runWindow("FT8", oneSecond);
